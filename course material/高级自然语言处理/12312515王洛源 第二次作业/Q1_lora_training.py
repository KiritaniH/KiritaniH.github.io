...
class LoRALinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        if x.dtype != self.base_weight.dtype:
            x = x.to(self.base_weight.dtype)
        result = torch.nn.functional.linear(x, self.base_weight, self.base_bias)
...
def inject_lora_modules(model: nn.Module, r: int = 8, alpha: int = 16, dropout: float = 0.0) -> int:
    ...
    for name, module in list(model.named_modules()):
        if isinstance(module, nn.Linear):
            if any(k in name for k in TARGET_LINEAR_KEYWORDS):
                parent_name = ".".join(name.split(".")[:-1])
                child_name = name.split(".")[-1]
                parent_module = model.get_submodule(parent_name) if parent_name else model

                lora_layer = LoRALinear(module, r, alpha, dropout).to(module.weight.device)
