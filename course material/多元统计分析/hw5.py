...
csv_data = """
国家和地区,100米,200米,400米,800米,1500米,5000米,10000米,马拉松
...
朝鲜民主主义人民共和国,10.91,21.94,47.30,1.85,3.77,14.13,29.67,130.87
...
美国,9.93,19.75,43.86,1.73,3.53,13.20,27.43,128.22
苏联,10.07,20.00,44.60,1.75,3.59,13.20,27.53,130.55
西萨摩亚,10.82,21.86,49.00,2.02,4.24,16.28,34.71,161.83
"""

df = pd.read_csv(StringIO(csv_data))

features = ["100米", "200米", "400米", "800米", "1500米", "5000米", "10000米", "马拉松"]
...
dendrogram(
    Z,
    labels=df["国家和地区"].values,
    leaf_rotation=90,
    leaf_font_size=8
)
plt.title("Hierarchical Clustering by Ward Method")
...
k = 4
df["Ward类"] = fcluster(Z, t=k, criterion="maxclust")

print("Ward 最小离差平方和法结果：")
for c in sorted(df["Ward类"].unique()):
    names = df.loc[df["Ward类"] == c, "国家和地区"].tolist()
    print(f"第{c}类：", names)

kmeans = KMeans(n_clusters=k, random_state=123, n_init=50)
df["KMeans类"] = kmeans.fit_predict(X_std) + 1

print("\nK-means 聚类结果：")
for c in sorted(df["KMeans类"].unique()):
    names = df.loc[df["KMeans类"] == c, "国家和地区"].tolist()
    print(f"第{c}类：", names)

print("\nWard 各类均值：")
print(df.groupby("Ward类")[features].mean().round(2))

print("\nK-means 各类均值：")
print(df.groupby("KMeans类")[features].mean().round(2))